...
class DialogueDataCollator:
    ...
    def process_one(self, messages, num_frame_tokens, return_length=False):
        # add video tokens template
        assert len(messages.questions) == 1
        #messages.questions[0] = '<Video>' + f'{"Video"*num_frame_tokens}' + '</Video>' + messages.questions[0]
        ...
        # print(len(flatten_message.input_ids))
        # print(self.tokenizer.decode(flatten_message.input_ids))
    ...
    def __call__(self, features):
        ...
        assert isinstance(features[0][0], (DatasetEntry, PretrainDatasetEntry))
        assert isinstance(features[0][1], torch.Tensor)
        ...
        for messages in features:
            ...
            num_frame_tokens = frames.shape[-2]
            
            flatten_message, label_mask, total_short_context_one = self.process_one(messages, num_frame_tokens)
        ...
        text_embeds = self.embedder(batch.input_ids.cuda()) # embed text
        merged_embeds = [] # text + video embeddings
        for i, input_id in enumerate(batch["input_ids"]):
            all_frames[i] = all_frames[i].squeeze().to(text_embeds.device, text_embeds.dtype) # some processing
            num_frame_tokens = all_frames[i].shape[-2]
            
            video_start_idx = None # where does the video template start
            for j in range(len(input_id)):
                if input_id[j:j+num_frame_tokens].tolist() == [15167]*num_frame_tokens: # 15167 is the token id for 'Video',     is the token for 'Image'
                    video_start_idx = j
                    break
            video_end_idx = video_start_idx + num_frame_tokens
            
            merged_embeds.append(torch.cat([text_embeds[i][:video_start_idx], all_frames[i], text_embeds[i][video_end_idx:]], dim=0)) # merge text and video embeddings
        batch["inputs_embeds"] = torch.stack(merged_embeds).cpu() # stack all embeddings along batch dimension
        del batch["input_ids"]
        
        # print(batch['inputs_embeds'][0, :15, 0])
        # print(all_frames[0][:15, 0])
