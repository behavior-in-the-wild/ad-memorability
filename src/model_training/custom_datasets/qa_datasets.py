...
class Lamem(Dataset):
    ...
    def __init__(self, cache_dir) -> None:
        super().__init__()
        os.makedirs(cache_dir, exist_ok=True)
        self.dataset= pd.read_csv('/path/to/scores.csv')
        self.dataset= self.dataset[:int(0.9*len(self.dataset))]
        
        prompt= 'This image was shown in a sequence of diverse images, assign a memorability score to this image based on how likely you think it is that a human will detect a repetition of the image a few minutes after being exposed to it'
        
        num_digits= 2
        self.pairs = []
        df= pd.read_csv('/path/to/features.csv')
        self.dataset= df.merge(self.dataset, right_on='image', left_on='image_name', how='right')
        self.dataset.fillna(' ', inplace=True)
        self.dataset['ques'] = prompt+ f'<Image>{"Image"*96}</Image>' + "The image can be described as " + self.dataset['caption'] + "and can be characterized by the keywords " + self.dataset['tags'].str.replace(' |', ',') + ". The emotion of the image is " + self.dataset['emotion'] + "The dominant colors in the image are " + self.dataset['colors'].apply(lambda x: ','.join(x)) + ". The tone of the image is " + self.dataset['Dominant_Tone'] + ". Return the memorability score of the video on a scale of 0 to 99 without any explanation. "
        self.dataset['ans'] = self.dataset['score'].apply(lambda x: str(math.ceil(float(x)*100-1)).zfill(num_digits))
        self.dataset['pairs'] = self.dataset.apply(lambda x: DatasetEntry(questions=[x['ques']], answers=[x['ans']]), axis=1)
        self.pairs = self.dataset['pairs'].tolist()
    ...
    def __getitem__(self, index) -> DatasetEntry:
        image= self.dataset.iloc[index]['image']
        name = image.split('/')[-1]
        name = name.split('.')[0]
        frame= torch.load('/path/to/tensors/'+name+'.pt')
   
        return self.pairs[index], frame
