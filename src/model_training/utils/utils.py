...
TOKENIZER_CONFIGS = {
    ...
    "mpt": TokenizerConfig(special_tokens=SpecialTokens("<|padding|>", "<|endoftext|>", "<|endoftext|>")),
    "Pajama": TokenizerConfig(special_tokens=SpecialTokens("<|endoftext|>", "<|endoftext|>", "<|endoftext|>")),
    "stable": TokenizerConfig(special_tokens=SpecialTokens("<|endoftext|>", "<|endoftext|>", "<|endoftext|>")),
...
def default_preprocess(eval_pred, ignote_negative_labels=True):
    ...
    mask = labels > 0
    with open('preds-labels.txt', 'a') as f:
        f.write(f'preds[mask]: {preds[mask]}\n')
        f.write(f'labels[mask]: {labels[mask]}\n')
...
def get_model(conf, tokenizer, pad_vocab_size_to_multiple_of=16, check_freeze_layer=True):
    ...
    print("Number of parameters: {}M".format(int(sum(p.numel() for p in model.parameters()) / 1e6)))
